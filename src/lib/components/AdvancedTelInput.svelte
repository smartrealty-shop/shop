<script lang="ts">
	...
	export let value = '';
	export let searchPlaceholder = 'Search'; // TODO translate
...
>
	<div class="flex" use:clickOutsideAction={closeOnClockOutside}>
		<button
			id="states-button"
					...
					{#each sortCountries(normalizedCountries, searchText) as country (country.id)}
						...
						<div id="country-{country.iso2}" role="option" aria-selected={isActive}>
							<button
								value={country.iso2}
								type="button"
								class="inline-flex py-2 px-4 w-full text-sm hover:bg-gray-100 active:bg-gray-800 overflow-hidden {isActive
									? 'bg-gray-600'
									: ''}"
								on:click={(e) => {
	...
	<!-- XXX: next line is about form correct data, dunno why value as var and value as prop of TelInput is not same -->
	...
	<TelInput
		...
		{options}
		required={true}
		class="input input-bordered p-2.5 rounded-l-none border-l-0 w-full bg-gray-50 text-gray-900"
	/>
	<!-- text-sm rounded-r-lg block w-full p-2.5 focus:outline-none border border-gray-300 border-l-gray-100 bg-gray-50 text-gray-900 -->
</div>
